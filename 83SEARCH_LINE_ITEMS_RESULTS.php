...
$search_SETUP1 = "CREATE  TABLE SEARCHINV (INVNO CHAR(7), INVDOC VARCHAR(30), INVSTAFF CHAR(3),INVCUST FLOAT(7),INVDATETIME DATETIME,INVUNITS FLOAT(7),INVDESCR VARCHAR(50),INVPRICE FLOAT(8,2),INVTOT FLOAT(10,2),PETNAME VARCHAR(20),INVDECLINE INT(1))" ;
$search_SETUP2 = "INSERT INTO SEARCHINV SELECT INVNO,INVDOC, INVSTAFF, INVCUST,INVDATETIME,INVUNITS,INVDESCR,INVPRICE,INVTOT,CONCAT(INVLGSM,' ',PETNAME) AS PETNAME,INVDECLINE FROM DVMINV WHERE INSTR(INVDESCR,'$company') <> 0 AND INVDECLINE = 0" ;
$search_SETUP3 = "INSERT INTO SEARCHINV SELECT INVNO,INVDOC, INVSTAFF, INVCUST,INVDATETIME,INVUNITS,INVDESCR,INVPRICE,INVTOT,CONCAT(INVLGSM,' ',PETNAME) AS PETNAME, INVDECLINE FROM ARYDVMI WHERE INSTR(INVDESCR,'$company') <> 0  AND INVDATETIME >= '$startdate[0]'  AND INVDATETIME <= '$enddate[0]'  AND INVDECLINE = 0";
$search_ARINVOI="SELECT DATE_FORMAT(INVDATETIME, '%Y/%m/%d') AS INVDTE,INVDOC,INVSTAFF, INVNO,INVUNITS,CONCAT(COMPANY, ',' , CONTACT) AS COMPANY,INVDESCR,INVTOT,PETNAME FROM SEARCHINV JOIN ARCUSTO ON SEARCHINV.INVCUST = ARCUSTO.CUSTNO WHERE INVDATETIME >= '$startdate[0]' AND INVDATETIME <= '$enddate[0]' ORDER BY INVDTE ASC, INVNO ASC ";
...
<title>LINE ITEM SEARCH FROM <?php echo $_GET['startdate'].' TO '.$_GET['enddate'] . ' ' . $doc ; ?></title>
<!-- InstanceEndEditable -->
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr id="prthospname">
    <td colspan="9" height="30" align="center" class="Verdana13B"><?php echo $hospname ; ?>
    </td>
    </tr>
     <tr id="prtpurpose">
    <td colspan="9" height="15" align="center" class="Verdana13">Search Line Items for <?php echo $company.'&nbsp&nbsp'; ; if ($startdate == $enddate) {echo $stdum ;} else {echo $stdum .' - '. $enddum ;} if ($doc != "0") {echo ' for ' . $doc ;}?><br />&nbsp;</td>
     </tr>
  <tr height="10" bgcolor="#000000" class="Verdana11Bwhite">
    <td width="60" align="left">Inv.#</td>
    <td width="101" align="right">Date</td>
    <td width="120" align="center">Doctor&nbsp;</td>
    <td width="6" align="right">&nbsp;St</td>
    <td width="187"align="center">Client</td>
    <td width="219"align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Sp/Patient</td>
    <td width="253" align="center">Item</td>
    <td width="30" align="left">Units</td>
  ...
  <tr id="'.$row_ARINVOI['INVNO'].'" onmouseover="highliteline(this.id,\'#DCF6DD\'); CursorToPointer(this.id);" onmouseout="whiteoutline(this.id)" onclick="window.open(\'../../IMAGES/CUSTOM_DOCUMENTS/INVOICE_PREVIEW2.php?file2search='.'ARINVOI'.'&invdte='.$row_ARINVOI['INVDTE'].'&invno='.$row_ARINVOI['INVNO'].'\',\'_blank\')">
    ...
    <td width="140" align="left" class="Verdana11Blue">'.$row_ARINVOI['INVDOC'].'</td>
    <td width="6" align="left" class="Verdana11Red">'.$row_ARINVOI['INVSTAFF'].'</td>
    ...
    <td width="20" align="center" class="Verdana13">'.$row_ARINVOI['INVUNITS'].'</td>
  ...
  <tr>
    ...
    <td height="22" align="right" valign="top" class="Verdana13B"><?php setlocale(LC_MONETARY, 'en_US'); echo $row_COUNT['ABC'] . ' items, ' . money_format('%(#10n',$row_NET['TOTAL']); ?></td>
    <td height="22" valign="top" class="Verdana13B">&nbsp;</td>
  </tr>
</table>
    </td>
