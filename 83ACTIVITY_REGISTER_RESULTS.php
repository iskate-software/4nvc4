...
$search_DVMINV="SELECT *, DATE_FORMAT(INVDATETIME, '%m/%d/%Y') AS INVDTE FROM $file2search WHERE INVDATETIME >= '$startdate[0]' AND INVDATETIME <= '$enddate[0]' ORDER BY INVNO,INVPET,INVMAJ,INVMIN";
$DVMINV=mysqli_query($tryconnection, $search_DVMINV) or die(mysqli_error($mysqli_link));
$row_DVMINV=mysqli_fetch_assoc($DVMINV);

$search_GRANDTOTAL="SELECT SUM(INVTOT) AS GRANDTOTAL FROM $file2search WHERE INVDATETIME >= '$startdate[0]' AND INVDATETIME <= '$enddate[0]'";
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr id="prthospname">
    ...
    <tr id="prtpurpose">
    <td colspan="8" height="15" align="center" class="Verdana13">Activity Register for <?php if ($startdate == $enddate) {echo $stdum ;} else {echo $stdum .' - '. $enddum ;}?></td>
    </tr>
...
echo '>
	...
	$query_CLIENT = "SELECT * FROM ARCUSTO WHERE CUSTNO = '".$row_DVMINV['INVCUST']."'";
